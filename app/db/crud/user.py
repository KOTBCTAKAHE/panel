...
async def check_and_register_hwid(
    db: AsyncSession,
    user: User,
    current_hwid: str,
) -> tuple[bool, Optional[str]]:
    """
    Check and register a new HWID for a user.
    
    If the user has an HWID limit set:
    1. If current HWID already exists in user's hwids list, allow access
    2. If it's a new HWID and limit not reached, add it
    3. If limit is reached, deny access
    
    Args:
        db: Database session
        user: The user object with hwids and hwid_limit fields
        current_hwid: The HWID to check
        
    Returns:
        tuple: (is_allowed: bool, error_message: Optional[str])
    """
    from app.utils.hwid import check_and_store_hwid
    
    # Initialize if needed
    if user.hwids is None:
        user.hwids = []
    
    # Check and store HWID
    is_allowed, error = await check_and_store_hwid(
        user_hwids=user.hwids,
...
async def reset_user_hwids(
    ...
    """
    Reset all HWIDs for a user (by admin request).
    
    This clears the list of registered devices for the user,
    allowing them to re-register on new devices.
    
    Args:
        db: Database session
        user: The user whose HWIDs should be reset
    """
    user.hwids = []
    db.add(user)
    await db.commit()
...
async def remove_hwid(
    db: AsyncSession,
    user: User,
    hwid_to_remove: str,
) -> bool:
    """
    Remove a specific HWID from user's registered devices.
    
    Args:
        db: Database session
        user: The user object
        hwid_to_remove: The HWID to remove
        
    Returns:
        bool: True if HWID was found and removed, False otherwise
    """
    if user.hwids is None:
        user.hwids = []
    
    if hwid_to_remove in user.hwids:
        user.hwids.remove(hwid_to_remove)
        db.add(user)
        await db.commit()
        return True
