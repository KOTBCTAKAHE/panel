...
@router.patch(
    ...
    response_model=UserResponse,
    responses={403: responses._403, 404: responses._404},
    summary="Set HWID limit for user",
    description="Set the maximum number of unique devices (HWIDs) that can access this user's subscription.",
)
async def set_user_hwid_limit(
    username: str,
    hwid_limit: int = Query(..., ge=0, description="Maximum devices (0 for unlimited, or 1-100)"),
    db: AsyncSession = Depends(get_db),
    admin: AdminDetails = Depends(get_current),
):
    """
    Set the HWID limit for a user.
    
    - **hwid_limit**: Maximum number of unique devices allowed (0 = unlimited)
    - Returns the updated user information
    """
    return await user_operator.set_user_hwid_limit(db, username, hwid_limit, admin)
...
@router.delete(
    ...
    response_model=UserResponse,
    responses={403: responses._403, 404: responses._404},
    summary="Reset user's registered HWIDs",
    description="Clear all registered hardware IDs for a user, allowing them to use new devices.",
)
async def reset_user_hwids(
    ...
    """
    Reset all registered HWIDs for a user.
    
    This action clears the list of known devices, allowing the user to re-register
    on new devices even if they had reached their HWID limit.
    """
    return await user_operator.reset_user_hwids(db, username, admin)


@router.get(
    "/{username}/hwid_info",
    responses={403: responses._403, 404: responses._404},
    summary="Get user's HWID information",
    description="Get detailed information about the user's registered devices and HWID limit.",
)
async def get_user_hwid_info(
    username: str,
    db: AsyncSession = Depends(get_db),
    admin: AdminDetails = Depends(get_current),
):
    """
    Get HWID information for a user.
    ...
    - **devices**: List of registered device HWID hashes
    """
    return await user_operator.get_user_hwid_info(db, username, admin)
