...
class UserOperation(BaseOperation):
    ...
    async def set_user_hwid_limit(
        ...
        Args:
            ...
            hwid_limit: Maximum number of unique devices (0 = unlimited)
        ...
        if hwid_limit < 0:
            await self.raise_error(message="HWID limit cannot be negative", code=400)
        
        db_user.hwid_limit = hwid_limit if hwid_limit > 0 else None
        db.add(db_user)
        await db.commit()
    ...
    async def reset_user_hwids(
        ...
        Returns:
            Updated UserResponse with empty HWID list
        """
        from app.db.crud.user import reset_user_hwids as crud_reset_hwid
        
        db_user = await self.get_validated_user(db, username, admin)
        await crud_reset_hwid(db, db_user)
        await db.refresh(db_user)
    ...
    async def get_user_hwid_info(
        ...
        Returns:
            Dictionary with HWID information including:
            - current_count: Number of registered devices
            - limit: HWID limit (or "unlimited")
            - available: Number of available slots
            - devices: List of registered HWID hashes
        """
        from app.utils.hwid import get_hwid_info
        
        db_user = await self.get_validated_user(db, username, admin)
        
        hwid_info = get_hwid_info(db_user.hwids, db_user.hwid_limit)
        
        return {
            "username": db_user.username,
            "current_count": hwid_info["current_count"],
            "limit": hwid_info["limit"],
            "available": hwid_info["available"],
            "devices": hwid_info["devices"],
        }
