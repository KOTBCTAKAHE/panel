...
from app.db.crud.user import get_user_usages, user_sub_update, check_and_register_hwid
...
class SubscriptionOperation(BaseOperation):
    ...
    async def user_subscription(
        ...
        if not sub_settings.disable_sub_template and is_browser_request:
            ...
            # Check HWID limit before allowing subscription access
            current_hwid = calculate_hwid(user_agent=user_agent)
            is_allowed, error_message = await check_and_register_hwid(db, db_user, current_hwid)
            if not is_allowed:
                await self.raise_error(message=error_message, code=403)
