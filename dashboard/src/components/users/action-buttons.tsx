...
import { Cat, Check, Copy, Cpu, EllipsisVertical, GlobeLock, HardDrive, Link2Off, ListStart, ListTree, Network, Pencil, PieChart, QrCode, RefreshCcw, Trash2, UserCog, Users } from 'lucide-react'
...
const ActionButtons: FC<ActionButtonsProps> = ({ user, isModalHost = true, renderActions = true }) => {
  ...
  const setHWIDModalOpen = useCallback((value: boolean) => setModalState({ isHWIDModalOpen: value }), [setModalState])
    ...
    <div onClick={renderActions ? (e => e.stopPropagation()) : undefined}>
      ...
      {isModalHost && (
        <>
          ...
          {/* HWIDManagementModal */}
          <HWIDManagementModal user={user} isOpen={isHWIDModalOpen} onOpenChange={setHWIDModalOpen} onSuccess={() => {
            invalidateUserMetricsQueries(queryClient)
          }} />
